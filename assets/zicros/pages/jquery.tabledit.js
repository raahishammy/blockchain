...
(function($) {
    ...
    $.fn.Tabledit = function(options) {
		...
		var status_code = {};
		status_code['New Quote'] = 'btn';
		status_code['New Booking'] = 'btn btn-danger';
		status_code['Searching For Driver'] = 'btn btn-pink';
		status_code['Assigned'] = 'btn btn-orange';
		status_code['Job Accepted'] = 'btn btn-purple';
		status_code['On The Way To Pick Up'] = 'btn btn-warning';
		status_code['Driver Arrived at Pickup Point'] = 'btn btn-teal';
		status_code['In Transit'] = 'btn btn-info';
		status_code['Driver Job Done'] = 'btn btn-success';
		status_code['Cancelled By Driver'] = 'btn btn-inverse';
		status_code['Cancelled By Passenger'] = 'btn btn-inverse';
		status_code['Cancelled By Admin'] = 'btn btn-inverse';
        var $table = this;
                ...
                editable: function() {
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
							...
							if(settings.columns.editable[i][1] == 'order_status'){
								 var span = '<span class="tabledit-span '+status_code[$(this).text()]+'">' + $(this).text() + '</span>';
							}else{
								// Create span element.
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
								...
								var uniq = $(this).parent('tr').attr('id');
                                var input = '<input id="yaho'+uniq+'" class="tabledit-input ' + settings.inputClass + '" type="text" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;" disabled>';
								var scpt = '<script>$("#yaho'+uniq+'").datetimepicker({format: "YYYY-MM-DD HH:mm", useCurrent: true,sideBySide: true,icons: {time: "fa fa-clock-o",date: "fa fa-calendar",up: "fa fa-chevron-up",down: "fa fa-chevron-down", previous: "fa fa-chevron-left",next: "fa fa-chevron-right",today: "fa fa-arrows ",clear: "fa fa-trash",close: "fa fa-times" }}).next().on(ace.click_event, function(){$(this).prev().focus();});</script>';
                           }
							
                            // Add elements and class "view" to table cell.
                            $(this).html(span + input + scpt);
                ...
                $(td).each(function() {
                    ...
                    if ($input.is('select')) {
                       // $(this).find('.tabledit-span').text($input.find('option:selected').text());
						if($input.attr('name') == 'order_status'){
							$(this).find('.tabledit-span').remove();
							var span = '<span class="tabledit-span '+status_code[$input.find('option:selected').text()]+'">' + $input.find('option:selected').text() + '</span>';
							$(this).prepend(span);
						}else{
							$(this).find('.tabledit-span').text($input.find('option:selected').text());
						}
        ...
        if (settings.editButton) {
            ...
            $table.on(settings.eventType, 'td.tabledit-view-mode', function(event) {
                if (event.handled !== true) {
                    event.preventDefault();
					var chkval = $table.find('.tabledit-input:visible').val();
