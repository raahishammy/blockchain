...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		{
			$this->error("Email is invalid. Please enter correct Email Address");
			return FALSE;
		}
		...
		$query = $this->aauth_db->get('ci_aauth_users');
		
		if($query->num_rows() == 0){
			$this->error("No User found with the given Email ID");
			return FALSE;
		}
		...
		if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) { 
			...
			$this->error("Acount Details are not valid. Please try again.");
			return FALSE;			
		}
	}
	...
	public function login_fast($user_id){
		...
		$query = $this->aauth_db->where(' (banned is NULL or banned=0) ');
		$query = $this->aauth_db->get('ci_aauth_users');
	...
	public function remind_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$data['last_reset_req_time'] 	= date('Y-m-d H:i:s');

			$this->aauth_db->where('email', $email);
			$this->aauth_db->update('ci_aauth_users', $data);
			...
			$mgClient = new Mailgun\Mailgun($this->CI->config->item('Mailgun')['ApiKey']);
			$domain = $this->CI->config->item('Mailgun')['Domain'];
			$frommail = $this->CI->config->item('Mailgun')['CorpEmail'];
			//$emaildata['email_message'] = $this->CI->lang->line('aauth_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $ver_code;
			
			//$emaildata['email_message'] = sprintf($this->CI->lang->line('aauth_email_reset_text'),site_url() . $this->config_vars['reset_password_link'] . $ver_code,site_url() . $this->config_vars['reset_password_link'] . $ver_code);
			$email_data['password_link']  = site_url() . $this->config_vars['reset_password_link'] . $ver_code;
			$email_data['user']  = $row;
			$email_data['template']  = "Reset";
			$new_html = $this->CI->load->view('email/reset-password-email',$email_data, true);
			
			
			$emaildata['HeaderEmail'] =  $this->CI->config->item('Mailgun')['CorpReplyToEmail'];
			if (ENVIRONMENT == 'production'){
				$subject = $this->CI->lang->line('aauth_email_reset_subject');
			}else{
				$subject = $this->CI->lang->line('aauth_email_reset_subject').' [Test]';
			}
			//$new_html = $this->CI->load->view('email/email-content',$emaildata, true);
			$result = $mgClient->sendMessage($domain, array(
				'from'    => 'Strides <'.$frommail.'>',
				'to'      => $row->email,
				'h:Reply-To'=>  ' < '.$this->CI->config->item('Mailgun')['CorpReplyToEmail'].' >',
				'subject' => $subject,
				'html'    =>  $new_html)
			);
			...
			$this->CI->email->sent_email([
				'email' => $row->email,
				'name' => '',
				'subject' => $this->CI->lang->line('aauth_email_reset_subject'),
				'html_body' => $this->CI->lang->line('aauth_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $ver_code
			]); */
	...
	public function account_verification($email){
		...
		if ($query->num_rows() > 0){
			...
			$domain = $this->CI->config->item('Mailgun')['Domain'];
			$frommail = $this->CI->config->item('Mailgun')['CorpEmail'];
			$email_data['verification_link']  = site_url() . 'account/verify/' . $ver_code;
			$email_data['user']  = $row;
			$new_html = $this->CI->load->view('email/account-verification-email',$email_data, true);
			
			$emaildata['HeaderEmail'] =  $this->CI->config->item('Mailgun')['CorpReplyToEmail'];
			if (ENVIRONMENT == 'production'){
				$subject = "Strides Account Creation : Account Verification";
			}else{
				$subject = "Strides Account Creation : Account Verification [Test]";
			}
			//$new_html = $this->CI->load->view('email/email-content',$emaildata, true);
			$result = $mgClient->sendMessage($domain, array(
				'from'    => 'Strides <'.$frommail.'>',
	...
	/**
	 * Set New password
	 * Emails user with link to Set a new password
	 * @param string $email Email for account to remind
	 * @return bool Remind fails/succeeds
	 */
	public function set_password($email){
		...
		if ($query->num_rows() > 0){
			...
			$ver_code = sha1(strtotime("now"));
			$datetime = get_user_datetime($row->opcountry,date("Y-m-d H:i:s"));
			$data['last_reset_req_time'] =  date('Y-m-d H:i:s', strtotime($datetime));
			$data['verification_code'] 		= $ver_code;
			...
			$mgClient = new Mailgun\Mailgun($this->CI->config->item('Mailgun')['ApiKey']);
			$domain = $this->CI->config->item('Mailgun')['Domain'];
			$frommail = $this->CI->config->item('Mailgun')['AccountEmail'];
			//$emaildata['email_message'] = $this->CI->lang->line('aauth_email_reset_text') . site_url() . $this->config_vars['reset_password_link'] . $ver_code;
			
			//$emaildata['email_message'] = sprintf($this->CI->lang->line('aauth_email_setpass_text'),site_url() . $this->config_vars['set_password_link'] . $ver_code,site_url() . $this->config_vars['set_password_link'] . $ver_code);
			
			$email_data['password_link']  = site_url() . $this->config_vars['set_password_link'] . $ver_code;
			$email_data['user']  = $row;
			$email_data['template']  = "Set";
			$new_html = $this->CI->load->view('email/reset-password-email',$email_data, true);
			
			
			$emaildata['HeaderEmail'] =  $this->CI->config->item('Mailgun')['AccountReplyEmail'];
			if (ENVIRONMENT == 'production'){
				$subject = $this->CI->lang->line('aauth_email_setpass_subject');
			}else{
				$subject = $this->CI->lang->line('aauth_email_setpass_subject').' [Test]';
			}
			//$new_html = $this->CI->load->view('email/set-new-pass',$emaildata, true);
			$result = $mgClient->sendMessage($domain, array(
				'from'    => 'Strides <'.$frommail.'>',
				'to'      => $row->email,
				'h:Reply-To'=>  ' < '.$this->CI->config->item('Mailgun')['AccountReplyEmail'].' >',
				'subject' => $subject,
				'html'    =>  $new_html)
			);
	...
	public function update_last_login($user_id = FALSE) {
		...
		return $this->aauth_db->update('ci_aauth_users', $data);
	}
	...
	public function update_login_attempts() {
		...
		// Update existing user login attempts
	...
	public function create_user($email, $pass, $username = FALSE) {
		...
		if ( $this->aauth_db->insert('ci_aauth_users', $data )){
	 ...
	 * @return bool Update fails/succeeds
	 */
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $username = FALSE, $extra = []) {
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE, $sort = FALSE) {
		...
		if ($group_par != FALSE) {
			...
			$this->aauth_db->select('*')
				->from('ci_aauth_users')
				->join($this->config_vars['user_to_group'], 'ci_aauth_users' . ".id = " . $this->config_vars['user_to_group'] . ".user_id")
	...
	public function get_passanger_role($user_id = FALSE){
		if ($user_id == FALSE)
			$user_id = $this->CI->session->userdata('id');
		
		$passenger = $this->CI->db->get_where('passenger', array('user_id'=>$user_id))->row();
		if(!empty($passenger)){
	...
	public function get_user_opcountry($user_id = FALSE){
		if ($user_id == FALSE)
			$user_id = $this->CI->session->userdata('id');
		
		$opcountry = $this->CI->db->get_where('aauth_users', array('id'=>$user_id))->row()->opcountry;
		return $opcountry;
	}
	public function is_staff($user_id = FALSE){
		if ($user_id == FALSE)
			$user_id = $this->CI->session->userdata('id');
		
		$role = $this->CI->db->get_where('aauth_users', array('id'=>$user_id))->row();
		if($role){
			$staff_roles = $this->CI->config->item('is_staff_role_code');
			foreach($staff_roles as $key => $val){
				if($role->user_role == $key){
					return true;
				}
			}
		}
	...
	public function send_verification($user_id){
		...
		if ($query->num_rows() > 0){
			...
			$this->CI->email->sent_email([
				...
				'html_body' => $this->CI->lang->line('aauth_email_verification_code') . $ver_code .
					$this->CI->lang->line('aauth_email_verification_text') . site_url() .$this->config_vars['verification_link'] . $user_id . '/' . $ver_code
			]);
	...
	public function delete_user($user_id) {
		...
		// delete user
		$this->aauth_db->where('id', $user_id);
		return $this->aauth_db->delete('ci_aauth_users');
	...
	/* OPcountry dropdown */
	public function opcountry_dropdown(){
		if($this->is_admin()){
			$op = 1;
		}else if($this->is_staff()){
			$op = $this->get_user_opcountry();
		}elseif($this->get_user_role() == 'Passenger'){
			$op = $this->get_user_opcountry();
		}else{
			$op = 1;
		}
		$opcountry = array();
		$regions = json_decode($this->get_system_var("region"));
		if(!empty($regions)){
			foreach($regions as $region){
				if(!empty($op) && $op != 1){
					if($region->country_code == $op){
						$opcountry[$region->country_code] = $region->name;
					}
				}else{
					$opcountry[$region->country_code] = $region->name;
				}
	...
	public function is_super_admin( $user_id = FALSE ) {
		...
		return $this->is_member($this->config_vars['super_admin_group'], $user_id);
	}
	//tested
	/**
	 * List groups
	...
	public function set_system_var( $key, $value ) {
		if ( ! $this->get_system_var($key, true) ) {
          $data = array(
				'key' => $key,
				'value' => $value,
			);
			return $this->CI->db->insert('aauth_system_variables' , $data);
		}
		else {
		$data = array(
				'key' => $key,
			...
			return $this->CI->db->update( 'aauth_system_variables', $data);
		}
	...
	public function unset_system_var( $key	) {
		...
		return $this->CI->db->delete( 'aauth_system_variables' );
	}
	 ...
	 * @return bool|string , FALSE if var is not set, the value of var if set
	 */
	public function get_system_var( $key , $count = false){

		$this->CI->db->where('key', $key);
		
		$query = $this->CI->db->get( 'aauth_system_variables');
	...
	public function list_system_vars(){
	...
	########################
	# Aauth Language Variables
	########################

	//tested
	/**
	 * Set Aauth Language Variable as key value
	 * if variable not set before, it will be set
	 * if set, overwrites the value
	 * @param string $key
	 * @param string $value
	 * @return bool
	 */
	public function set_lang_var( $lang,$key, $value, $lang_key_type = 0 ) {

		// if var not set, set
		if ( ! $this->get_lang_var($lang,$key) ) {

			$data = array(
				'lang' => $lang,
				'key' => $key,
				'value' => $value,
				'lang_key_type' => $lang_key_type
			);

			return $this->CI->db->insert( 'aauth_lang_variables' , $data);
		...
		// if var already set, overwrite
			...
			$this->CI->db->where(array('key'=> $key,'lang'=>$lang) );
			return $this->CI->db->update( 'aauth_lang_variables', $data);
		}
	...
	//tested
	/**
	 * Unset Aauth Language Variable as key value
	 * @param string $key
	 * @return bool
	 */
	public function unset_lang_var( $key	) {

		$this->CI->db->where('key', $key);

		return $this->CI->db->delete( 'aauth_lang_variables' );
	}

	//tested
	/**
	 * Get Aauth Language Variable by key
	 * Return string of variable value or FALSE
	 * @param string $key
	 * @return bool|string , FALSE if var is not set, the value of var if set
	 */
	public function get_lang_var($lang, $key ){
		$query = '';
		$query = $this->CI->db->get_where( 'aauth_lang_variables', array('key'=> $key,'lang'=>$lang));
		// if variable not set
		if ($query->num_rows() < 1) { 
			return FALSE;
		}else {
	 ...
	 /**
	 * List Language Variable Keys
	 * Return array of variable keys or FALSE
	 * @return bool|array , FALSE if var is not set, the value of var if set
	 */

	public function list_lang_var_keys(){
		$query = $this->CI->db->select('key');
		$query = $this->CI->db->get( 'aauth_lang_variables' );
		// if variable not set
		if ($query->num_rows() < 1) { return FALSE;}
		else {
			return $query->result();
	...
	public function list_lang_vars(){
		$query = $this->CI->db->select('*');
		$query = $this->CI->db->get( 'aauth_lang_variables' );
		// if variable not set
		if ($query->num_rows() < 1) { return FALSE;}
	...
	public function generate_totp_onecode($secret){
		$this->CI->load->helper('googleauthenticator');
		$ga = new PHPGangsta_GoogleAuthenticator();
	...
	// added by subh
	public function get_passenger_company_id($userid = FALSE){
		if ($userid == FALSE)
			$userid = $this->CI->session->userdata('id');

		$this->CI->db->where("user_id",$userid);
		$query = $this->CI->db->get("passenger");
	...
	public function get_user_company_id($userid = FALSE){
		...
		$query = $this->CI->db->get("aauth_users")->row();
		if(!empty($query)){
	...
	public function get_users_of_company($passenger_company_id){
		
		$this->CI->db->select("group_concat(user_id) as users");
		$this->CI->db->where("passenger_company_id",$passenger_company_id);
		$query = $this->CI->db->get("passenger");
		return $query->result_array();

	}
	public function get_driver_info($driver_id){
		$this->CI->db->select('*,driver.id as driver_id')
			->from("driver")
			->join("aauth_users", "driver.user_id = aauth_users.id")
			->where("driver.id",$driver_id);
		$query = $this->CI->db->get();
		return $query->row_array();
	}
	
	// check if need to rest password
	public function check_requested_reset($email) {
	...
	public function check_requested_change($email) {
		...
		if($query->num_rows() >0){
			$row = $query->row();
			if( isset($row->is_needchangepass) and $row->is_needchangepass == "1" ){
	...
	public function can_view_pricing($user_id = FALSE){
		if ($user_id == FALSE)
			$user_id = $this->CI->session->userdata('id');
		if($company = $this->get_passenger_company_id($user_id)){
				$passenger = $this->CI->db->get_where('passenger_company', array('id'=>$company['passenger_company_id']))->row();		
				if(!empty($passenger) and $passenger->can_view_pricing==1){
					$passenger_info = $this->CI->db->get_where('passenger', array('user_id'=>$user_id))->row();
					return !!($passenger_info && $passenger_info->can_view_pricing);
				}
				return !!($passenger && $passenger->can_view_pricing);
		}
	}
